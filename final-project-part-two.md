...
|Is there anything you would change, do differently, or add?|Instead of having text that repeats what the graphs show, do more concise bullet points or let graphs speak for themselves if it's self-explanatory. For example, descriptive title for graphs can be enough instead of having to add text to explain what is obvious to audience. Have less text heavy components. "Seems more like a report right now, change it to read like a presentation."|"Have titles for each section, like chapters in a book. Have an overall title/big header to physically separate each chunk of information and graph."|"Add narratives from real people who struggle with student loans because it can make the issue seem more real to policymakers and help them symphasize."|
